...
class AdminMapper
{
    ...
    public function getAllUsers($id):array {
        try {
            $stmt = $this->database->connect()->prepare('SELECT * FROM Base_users where id not like :id');
    ...
    public function deleteUser($id_user):void {
        ...
        try {
            $stmt = $this->database->connect()->prepare('DELETE FROM Base_users WHERE id = :id_user;');
